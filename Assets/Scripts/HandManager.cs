...
public class HandManager : MonoBehaviour
{
    ...
    public async UniTask RemoveCard(Card card, Vector3 position, CardLocation location)
    {
        var token = this.GetCancellationTokenOnDestroy();
        CardView cardView = GetCardView(card);
        handCards.Remove(cardView);
        UpdateCardPosition();
        if (location is CardLocation.DropZone)
        {
            await cardView.transform.DOMove(position, 0.5f).ToUniTask(cancellationToken: token);
